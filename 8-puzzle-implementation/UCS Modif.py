        ...
        for row, col in [(zero_row - 1, zero_col), (zero_row + 1, zero_col), (zero_row, zero_col - 1), (zero_row, zero_col + 1)]:
            if row >= 0 and row < 3 and col >= 0 and col < 3:
                ...
                if tuple(map(tuple, child_state)) not in explored:
                    cost = len(path) + 1
                    frontier.put((cost, child_state, path + [(row, col)]))

    return intermediate_states

# Function to run UCS multiple times and calculate average nodes visited and time
def run_ucs_multiple_times(initial_state, goal_state, num_times):
    total_nodes_visited = 0
    total_run_time = 0

    for _ in range(num_times):
        start_time = time.perf_counter()
        intermediate_states = ucs(initial_state, goal_state)
...
# Define the number of times to run UCS with different initial configurations
num_times = 10  # You can change this to any desired number

# Run UCS multiple times and calculate average nodes visited and time
average_nodes_visited, average_run_time = run_ucs_multiple_times(initial_state, goal_state, num_times)
