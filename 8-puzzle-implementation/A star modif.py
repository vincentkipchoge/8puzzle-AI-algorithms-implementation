...
# Function to run A* with different initial configurations
def run_a_star_with_random_initial_configurations(goal_state, num_configs):
    total_nodes_visited = 0
    total_run_time = 0

    for _ in range(num_configs):
        initial_config = random.sample(range(9), 9)
        initial_state = [[initial_config[3 * i + j] for j in range(3)] for i in range(3)]
    ...
    average_nodes_visited = total_nodes_visited / num_configs
    average_run_time = total_run_time / num_configs

    print(f'Average nodes visited: {average_nodes_visited}')
    print(f'Average run time (seconds): {average_run_time}')

    # Start the animation
    animate_solution(0, intermediate_states)

# Function to animate the solution with highlighted moves
def animate_solution(step, intermediate_states):
    if step < len(intermediate_states):
        ...
        window.after(1000, animate_solution, step + 1, intermediate_states)  # Update every 1000 milliseconds (1 second)

# Solve the 8-puzzle problem with random initial configurations
goal_state = [[1, 2, 3], [4, 5, 6], [7, 8, 0]]
run_a_star_with_random_initial_configurations(goal_state, num_configs=10)
